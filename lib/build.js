...
async function buildXlsx(input) {
  ...
  // fitToWidth=1 은 표가 인쇄 폭보다 넓으면 줄이지만, 표가 인쇄 폭보다 좁을 때도 그만큼
  // '늘려서' 폭을 꽉 채운다. 표가 원래 좁게 만들어진 양식(여기서는 6.35in 표를 6.85in
  // 폭에 억지로 채우면 108%)에서는 이 확대가 행 높이까지 같이 키워서, 한 페이지에
  // 들어가야 할 블록이 페이지 중간에 걸려 둘로 쪼개진다. 표가 실제로 넓을 때만 줄인다.
  const contentW = form.cols.reduce((t, w) => t + Math.round(w * (form.pxPerChar || 8) + 5), 0) / 96;
  const printW = 8.2677 - (form.margins ? form.margins.lr : 0.7) * 2;
  const needsShrink = contentW > printW + 0.02;
  ...
  ws.pageSetup = Object.assign({}, ws.pageSetup, {
    ...
    fitToPage: needsShrink, fitToWidth: needsShrink ? 1 : 0, fitToHeight: 0,
    scale: needsShrink ? undefined : 100,              // 좁으면 확대하지 말고 원래 크기 그대로
  });
  // 수동 나누기를 빼고 엑셀 자동 나누기에 맡겨 봤더니, 뒤쪽 블록으로 갈수록 우리 계산과
  // 엑셀 자체 계산이 미세하게 어긋나(예: 60행이 아니라 59행에서 끊음) 블록(사진+표) 하나가
  // 페이지 경계에서 반으로 잘렸다. "페이지 나누기 미리보기에 선이 겹쳐 보인다"는 것보다
  // 사진이 잘리는 게 훨씬 나쁘므로, 수동 나누기는 블록 수와 상관없이 항상 넣는다.
  // 마지막 열 번호를 직접 넘긴다 (기본값 16838은 엑셀 최대 열 16383을 넘어 파일 손상으로 판정된다)
  // 페이지 나누기는 마지막 행이 아니라 다음 묶음의 첫 행 앞에 둔다.
  // 마지막 행에 걸면 그 행의 아래 테두리와 내용이 다음 페이지 경계에서 잘린다.
  for (let b = fpb; b < blocks; b += bpp) ws.getRow(bs + b * form.block).addPageBreak(0, 16384);
