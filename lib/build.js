...
function resolveForm(input = {}) {
  const requestedId = input.form || (input.formDef && input.formDef.id);
  const builtIn = BUILTIN_FORM_IDS.has(requestedId);
  return {
    builtIn,
    form: builtIn ? findForm(requestedId) : (input.formDef || findForm(input.form)),
  };
}
...
function normalizeForLibreOffice(ws, form = {}, itemCount = 0) {
  // Some Excel-authored forms carry fitToWidth/fitToHeight values while fitToPage is off.
  // Excel still keeps them on one page through printer-specific metrics, but LibreOffice
  // ignores those values and spills a tiny right/bottom edge onto three extra pages.
  // This is applied only to forms measured with loFitToPages and only to the PDF copy.
  if (form.loFitToPages) {
    const blocks = Math.max(1, Math.ceil(Math.max(1, itemCount) / positiveNumber(form.perPage, 1)));
    const pdfScale = Math.min(400, Math.max(10, positiveNumber(form.loPdfScale, 85)));
    ...
    // In scale mode LibreOffice respects manual breaks. Keep every repeated form block
    // intact, while the measured scale ensures one complete block fits on one A4 page.
    const blockStart = positiveNumber(form.blockStart, 1);
    const blockSize = positiveNumber(form.block, 1);
    ws.rowBreaks = [];
    for (let page = 1; page < blocks; page++) {
      ws.getRow(blockStart - 1 + page * blockSize).addPageBreak();
    }
  }
  // Calibration belongs to the form. New uploads default to 1.0 until measured.
  const colWidthFix = positiveNumber(form.loColWidthFix, 1);
  const rowHeightFix = positiveNumber(form.loRowHeightFix, 1);
  ws.columns.forEach(c => { if (c.width) c.width = c.width * colWidthFix; });
  ws.eachRow({ includeEmpty: true }, row => {
    if (row.height) row.height = row.height * rowHeightFix;
...
function cloneCellValue(value) {
  ...
  const copy = {};
  for (const [key, item] of Object.entries(value)) copy[key] = cloneCellValue(item);
  return copy;
}
...
function clonePage(ws, form, merges, pageIndex) {
  ...
  for (let r = bs; r <= bs + form.block - 1; r++) {
    ...
    for (let c = 1; c <= form.cols.length + 2; c++) {
      ...
      // Preserve formulas, numbers, dates, rich text, and hyperlinks in repeated pages.
      dc.value = cloneCellValue(sc.value);
    }
...
async function buildXlsx(input) {
  ...
  const resolved = resolveForm(input);
  const form = resolved.form;
  const site = resolved.builtIn ? BUILTIN_SITE : input.site;

  const wb = new ExcelJS.Workbook();
  if (input.template && !resolved.builtIn) await wb.xlsx.load(Buffer.from(input.template, 'base64'));
  ...
  if (site && form.site) {
    const text = (form.site.prefix || '') + site;
  ...
  // The form author controls paper, scale, fit, and manual page-break settings.
  // Preserve those settings; only extend the printable range for added photo blocks.
  ws.pageSetup = Object.assign({}, ws.pageSetup, {
    // 열 앞의 $ 는 ExcelJS가 붙인다. 여기서 붙이면 '$$A$1' 이 되어 파일이 손상된다.
    printArea: `A1:${lastCol}${rows}`,
  });
...
module.exports = { buildXlsx, formatDate, forms, resolveForm, stripPhotos, normalizeForLibreOffice };
