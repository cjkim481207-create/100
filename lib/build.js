...
function normalizeForLibreOffice(ws, form = {}, itemCount = 0) {
  ...
  if (form.loFitToPages) {
    ...
    // Treat every repeated block as a separate print area. A single N-pages-tall area
    // can be divided through a block because only the first block has document headers.
    // Separate areas let LibreOffice fit each complete form block to exactly one page.
    const lastCol = colName((form.cols || []).length || ws.columnCount || 1);
    const blockStart = positiveNumber(form.blockStart, 1);
    const blockSize = positiveNumber(form.block, 1);
    const printAreas = [`A1:${lastCol}${blockStart - 1 + blockSize}`];
    for (let page = 1; page < blocks; page++) {
      const start = blockStart + page * blockSize;
      printAreas.push(`A${start}:${lastCol}${start + blockSize - 1}`);
    }
    ws.pageSetup.printArea = printAreas.join('&&');
    ws.rowBreaks = [];
  }
