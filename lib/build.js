...
function normalizeForLibreOffice(ws, form = {}, itemCount = 0) {
  ...
  if (form.loFitToPages) {
    ...
    // Fitting the whole range to N pages does not guarantee that LibreOffice cuts
    // exactly between repeated form blocks, especially because only page 1 has a header.
    // Explicit breaks keep every cloned form block intact on its own PDF page.
    ws.rowBreaks = [];
