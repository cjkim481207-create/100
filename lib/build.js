...
async function buildXlsx(input) {
  ...
  // 1페이지 블록을 페이지 수만큼 복제한다 (양식에 다른 형태의 페이지가 섞여 있어도 첫 장 기준으로 통일)
  for (let p = 1; p < pages; p++) clonePage(ws, form, merges, p);

  const dateText = formatDate(input.date);
  const slots = pages * form.perPage;
  for (let i = 0; i < slots; i++) {
    const p = Math.floor(i / form.perPage), s = i % form.perPage, base = p * form.block;
    const slot = form.slots[s];
    const it = items[i];
    const val = Object.assign({}, it && it.fields, { date: it ? dateText : null });
  ...
  if (input.site && form.site) {
    ...
    if (form.site.row < bs) {                       // 문서 머리글에 있으면 맨 위 한 번만
      ...
      for (let p = 0; p < pages; p++) ws.getCell(p * form.block + form.site.row, form.site.col).value = text;
    }
  }

  const rows = (form.blockStart || 1) - 1 + pages * form.block;
  ...
  for (let p = 1; p < pages; p++) ws.getRow((form.blockStart || 1) - 1 + p * form.block).addPageBreak();
