...
function blockMerges(ws, block) {
  const out = [];
  for (const m of (ws.model.merges || [])) {
    const p = /^([A-Z]+)(\d+):([A-Z]+)(\d+)$/.exec(m);
    if (!p) continue;
    const r1 = +p[2], r2 = +p[4];
    if (r2 <= block) out.push([r1, colNum(p[1]), r2, colNum(p[3])]);
  }
  return out;
}
...
function clonePage(ws, form, merges, pageIndex) {
  const off = pageIndex * form.block;
  for (let r = 1; r <= form.block; r++) {
...
async function buildXlsx(input) {
  ...
  const form = findForm(input.form);

  const wb = new ExcelJS.Workbook();
  await wb.xlsx.readFile(path.join(DIR, form.template));
  const ws = wb.worksheets[0];

  const merges = blockMerges(ws, form.block);
  const pages = Math.ceil(items.length / form.perPage);
  const built = Math.max(1, Math.floor((ws.rowCount || form.block) / form.block));
  for (let p = built; p < pages; p++) clonePage(ws, form, merges, p);

  const dateText = formatDate(input.date);
  const slots = Math.max(pages, built) * form.perPage;
  for (let i = 0; i < slots; i++) {
    ...
    for (const line of slot.rows) {
      for (const cell of line.cells) {
        ...
        ws.getCell(base + line.row, cell.cols[0]).value = val[cell.field] || null;
      }
    }
    if (!it) continue;
    ...
    ws.addImage(id, {
      tl: {
        nativeCol: c0 - 1, nativeColOff: xOff * EMU,
        nativeRow: base + r0 - 1, nativeRowOff: yOff * EMU,
      },
  ...
  if (input.site && form.site) {
    for (let p = 0; p < pages; p++) {
      ws.getCell(p * form.block + form.site.row, form.site.col).value =
        (form.site.prefix || '') + input.site;
    }
  }

  const rows = pages * form.block, lastCol = String.fromCharCode(A + form.cols.length - 1);
  ws.pageSetup = Object.assign({}, ws.pageSetup, {
    printArea: `A1:${lastCol}${rows}`,
    orientation: 'portrait',
    fitToPage: true, fitToWidth: 1, fitToHeight: 1,
  });
  for (let p = 1; p < pages; p++) ws.getRow(p * form.block).addPageBreak();
...
module.exports = { buildXlsx, formatDate, forms };
