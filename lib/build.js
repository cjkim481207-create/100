...
const BOX = [[4, 13], [18, 27]];           // 사진박스 (시작행, 끝행)
const INF = [[15, 16], [29, 30]];          // [위치/일자 행, 내용/비고 행]
const PHOTO_COLS = [1, 9];                 // 박스 테두리 범위 A~I
const INSET = 12;                          // 사진과 테두리 사이 여백 (96dpi 픽셀)
const DEFAULT_COL_W = 8.43;
const PX_PER_CHAR = 8;                     // 맑은 고딕 11pt 기준 1문자 = 8px
...
const colPx = (ws, c) => Math.round((ws.getColumn(c).width || DEFAULT_COL_W) * PX_PER_CHAR + 5);
...
async function buildXlsx(input) {
  ...
  for (let i = 0; i < slots; i++) {
    ...
    const scale = Math.min((boxW - INSET * 2) / it.w, (boxH - INSET * 2) / it.h);
