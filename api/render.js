...
const RENDER_CONFIGURED = !!(RENDER_URL && RENDER_KEY);

// 사진대지 xlsx를 실제 엑셀 엔진(LibreOffice)으로 렌더한 PDF/PNG를 돌려준다.
// 화면 미리보기는 이걸 흉내 낸 그림이라 다를 수 있지만, 여기서 나온 결과는 곧 인쇄 결과 그 자체다.
module.exports = async (req, res) => {
  if (req.method === 'GET') { res.status(200).json({ ok: RENDER_CONFIGURED }); return; }
  if (req.method !== 'POST') { res.status(405).json({ error: 'POST만 지원합니다.' }); return; }
  if (!RENDER_CONFIGURED) { res.status(503).json({ error: '정밀 PDF 변환 서버 또는 인증 키가 설정되지 않았습니다.' }); return; }
  try {
